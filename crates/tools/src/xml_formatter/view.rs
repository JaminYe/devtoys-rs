...
impl XmlFormatterView {
    ...
    fn reformat(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        match format_xml(&source, self.indent, self.new_line_on_attributes) {
            Ok(formatted) => {
                self.error = None;
                self.output.update(cx, |output, cx| {
            ...
            Err(_) => {
                self.error = Some(SharedString::from("非法 XML"));
                self.output.update(cx, |output, cx| {
                    output.set_value(String::new(), window, cx);
                });
            }
...
impl ReceivesData for XmlFormatterView {
    fn on_data_received(
        &mut self,
        payload: &str,
        window: &mut Window,
...
impl Render for XmlFormatterView {
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        Switch::new("newline-on-attributes")
                            .label("属性换行")
                            .checked(self.new_line_on_attributes)
                            .on_click(cx.listener(|this, checked, window, cx| {
                                this.new_line_on_attributes = *checked;
                                this.reformat(window, cx);
                            })),
                    )
                    .child(
